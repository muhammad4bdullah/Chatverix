...
saveProfileBtn.onclick = async ()=>{
  ...
  await setDoc(doc(db,"users",currentUser.uid),{nickname,avatarUrl,lastSet:Date.now()},{merge:true});
...
document.getElementById('create-chat').onclick = async ()=>{
  ...
  const roomId=randomString(8), password=randomString(6);
  currentRoomId=roomId; chatsToday++;
  await setDoc(doc(db,"rooms",roomId),{password,createdBy:currentUser.uid,members:[nickname]});
  roomIdDisplay.textContent=`Room ID: ${roomId}`;
  roomPassDisplay.textContent=`Password: ${password}`;
  updateRoomLink(roomId,password);
  listenMessages();
